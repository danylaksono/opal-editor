...
fn normalize_identifier(value: &str) -> Result<Identifier, String> {
    ...
    if doi.starts_with("10.") && doi.contains('/') {
        return Ok(Identifier::Doi(doi.to_string()));
    }
    let isbn: String = trimmed
        .chars()
        .filter(|character| character.is_ascii_digit() || *character == 'X' || *character == 'x')
        .collect();
    if isbn.len() == 10 || isbn.len() == 13 {
        return Ok(Identifier::Isbn(isbn.to_uppercase()));
    }
    let arxiv = lowered
    ...
    let arxiv = arxiv
        .rsplit_once('v')
        .filter(|(_, version)| version.chars().all(|value| value.is_ascii_digit()))
        .map(|(base, _)| base)
        .unwrap_or(arxiv);
    if arxiv.contains('.') || arxiv.contains('/') {
        return Ok(Identifier::Arxiv(arxiv.to_string()));
    }
...
fn cache_key(identifier: &Identifier) -> String {
    match identifier {
        Identifier::Doi(value) => format!("doi:{value}"),
        Identifier::Isbn(value) => format!("isbn:{value}"),
        Identifier::Arxiv(value) => format!("arxiv:{value}"),
    }
}
...
async fn lookup_doi(client: &reqwest::Client, doi: &str) -> Result<CitationCandidate, String> {
    ...
    if response.status().is_success() {
        ...
        if raw.trim_start().starts_with('@') {
            return Ok(CitationCandidate {
                provider: "doi".into(),
                attribution: "DOI content negotiation".into(),
                identifier: doi.into(),
                entry_type: "article".into(),
                title: bib_field(&raw, "title"),
                authors: bib_field(&raw, "author")
                    .split(" and ")
                    .map(str::to_string)
                    .filter(|value| !value.is_empty())
                    .collect(),
                year: bib_field(&raw, "year"),
                journal: bib_field(&raw, "journal"),
                publisher: bib_field(&raw, "publisher"),
                doi: doi.into(),
                isbn: String::new(),
                arxiv_id: String::new(),
    ...
    let year = message["issued"]["date-parts"][0][0]
        ...
        .unwrap_or_default();
    Ok(CitationCandidate {
        provider: "crossref".into(),
        attribution: "Crossref REST API".into(),
        identifier: doi.into(),
        entry_type: message["type"].as_str().unwrap_or("article").into(),
        title: message["title"][0].as_str().unwrap_or("").into(),
        authors,
        year,
        journal: message["container-title"][0].as_str().unwrap_or("").into(),
        publisher: message["publisher"].as_str().unwrap_or("").into(),
        doi: doi.into(),
        isbn: String::new(),
        arxiv_id: String::new(),
        url: message["URL"].as_str().unwrap_or("").into(),
        raw_metadata: raw,
        from_cache: false,
    })
}
...
async fn lookup_isbn(client: &reqwest::Client, isbn: &str) -> Result<CitationCandidate, String> {
    ...
    *last = Some(Instant::now());
    ...
    if book.is_null() {
        return Err("Open Library did not return a matching edition".to_string());
    }
    ...
    Ok(CitationCandidate {
        ...
        attribution: "Open Library ISBN API".into(),
        identifier: isbn.into(),
        entry_type: "book".into(),
        title: book["title"].as_str().unwrap_or("").into(),
        authors,
        year,
        journal: String::new(),
        publisher: book["publishers"][0]["name"].as_str().unwrap_or("").into(),
        doi: String::new(),
        isbn: isbn.into(),
        arxiv_id: String::new(),
        url: book["url"].as_str().unwrap_or("").into(),
        raw_metadata: raw,
        from_cache: false,
    })
}
...
fn xml_text(source: &str, tag: &str) -> String {
    ...
    source
        .find(&open)
        .and_then(|start| {
            source[start + open.len()..].find(&close).map(|end| {
                source[start + open.len()..start + open.len() + end]
                    .trim()
                    .replace(['\n', '\r'], " ")
            })
        })
...
async fn lookup_arxiv(client: &reqwest::Client, arxiv: &str) -> Result<CitationCandidate, String> {
    ...
    let entry = raw
        .split("<entry>")
        .nth(1)
        .and_then(|value| value.split("</entry>").next())
        .ok_or("arXiv did not return a matching record")?;
    let authors = entry
        .split("<author>")
        .skip(1)
        .map(|value| xml_text(value, "name"))
        .filter(|value| !value.is_empty())
        .collect();
    let published = xml_text(entry, "published");
    Ok(CitationCandidate {
        provider: "arxiv".into(),
        attribution: "arXiv API; data provided by arXiv".into(),
        identifier: arxiv.into(),
        entry_type: "article".into(),
        title: xml_text(entry, "title"),
        authors,
        year: published.get(0..4).unwrap_or("").into(),
        journal: "arXiv preprint".into(),
...
pub async fn lookup_reference(
    identifier: String,
    refresh: Option<bool>,
) -> Result<CitationCandidate, String> {
    let normalized = normalize_identifier(&identifier)?;
    let key = cache_key(&normalized);
    let mut cache = read_cache();
    if !refresh.unwrap_or(false) {
        if let Some(entry) = cache.get(&key) {
            if now().saturating_sub(entry.cached_at) < CACHE_SECONDS {
    ...
    let candidate = match &normalized {
        Identifier::Doi(value) => lookup_doi(&client, value).await?,
        Identifier::Isbn(value) => lookup_isbn(&client, value).await?,
        Identifier::Arxiv(value) => lookup_arxiv(&client, value).await?,
    };
    cache.insert(
        key,
        CacheEntry {
...
mod tests {
    use super::*;
    #[test]
    fn normalizes_supported_identifiers() {
        assert_eq!(
            normalize_identifier("https://doi.org/10.1000/ABC").unwrap(),
            Identifier::Doi("10.1000/abc".into())
        );
        assert_eq!(
            normalize_identifier("978-0-306-40615-7").unwrap(),
            Identifier::Isbn("9780306406157".into())
        );
        assert_eq!(
            normalize_identifier("arXiv:2401.12345v2").unwrap(),
    ...
    fn parses_bibtex_fields() {
        assert_eq!(
            bib_field("@article{x, title = {Hello World}}", "title"),
            "Hello World"
        );
    }
    #[test]
    ...
    #[test]
    fn cache_expiry_is_thirty_days() {
        assert!(CACHE_SECONDS == 2_592_000);
    }
}
