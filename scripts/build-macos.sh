...
echo "==> Building Opal $TAG for macOS ($TARGET)"
...
APP_PATH="apps/desktop/src-tauri/target/$TARGET/release/bundle/macos/Opal.app"
  ...
  if [ -f "$LATEST_JSON" ]; then
    ...
    node -e "
      ...
      data.platforms['darwin-aarch64'] = {
        signature: \`$SIGNATURE\`,
        url: 'https://github.com/danylaksono/opal-editor/releases/download/$TAG/Opal-macOS.app.tar.gz'
      };
...
if [ -n "${UPDATE_TAR:-}" ]; then
  RENAMED_TAR="apps/desktop/src-tauri/target/Opal-macOS.app.tar.gz"
